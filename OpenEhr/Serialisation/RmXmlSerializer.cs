...
namespace OpenEhr.Serialisation
{
    ...
    public class RmXmlSerializer
    {
        ...
        /// <summary>
        /// XML serialisation to TextWriter - Criado para compatibilidade com EHR Server
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="version"></param>
        static public void SerializeEhrServer<T>(XmlWriter writer, Version<T> version)
            where T : class
        {
            Check.Require(writer != null, "writer type must not be null");
            Check.Require(version != null, "version type must not be null");

            writer.WriteStartElement("versions", OpenEhrNamespace);

            string openEhrPrefix = UseOpenEhrPrefix(writer);
            string xsiPrefix = UseXsiPrefix(writer);

            writer.WriteStartElement(openEhrPrefix, "version", OpenEhrNamespace);
            writer.WriteAttributeString(xsiPrefix, "type", XsiNamespace, openEhrPrefix + "ORIGINAL_VERSION");
